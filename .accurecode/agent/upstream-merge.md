...
**Do not load the `accurecode-merge-minimizer` skill.** That skill is for
authoring new Accure changes against shared upstream files; during an upstream
merge it gives the wrong guidance (it nudges toward extracting Accure logic out
...
- does a `accurecode_change` marker in HEAD encode a bug fix, a feature, or a
  defensive check?
...
When HEAD includes a non-obvious Accure-specific wrapper (e.g. a helper in
`packages/opencode/src/accurecode/`), find out why it exists before deciding to
keep or bypass it:

```bash
git log --all --oneline -S "<symbol>" -- packages/opencode/src/accurecode/
git log --all --oneline -- <accure-file>
```
...
- one-sentence rationale (what Accure behaviour is preserved, what upstream
...
3. State which Accure behaviour must survive and why (reference PR numbers /
   `accurecode_change` comments when possible).
...
- `.worktrees/opencode-merge/accure-main` — Accure base snapshot
- `.worktrees/opencode-merge/auto-merge` — automated merge snapshot (original
  conflict reference)

Apply in order:

- prefer upstream code and architecture whenever compatible with Accure behaviour
- preserve Accure-specific behaviour marked with `accurecode_change`
- keep `accurecode_change` markers around Accure-specific code in shared opencode
  files
- when upstream refactors a region that HEAD had annotated with
  `accurecode_change`, **check whether the marker encodes a bug fix or a feature
  ...
  `Effect.gen` block.
- when a `take-theirs` drops a line that was the target of a Accure pre-filter,
  the upstream line may be actively wrong for Accure — e.g. an inner `continue`
  filter whose condition collides with an outer filter Accure added. Re-read the
  surrounding 20 lines before committing to `take-theirs`.
- if Accure-specific code must be refactored to fit new upstream architecture,
  explain the refactor in the final summary
- if upstream moved the relevant logic to another file, port the Accure behaviour
  there and list both paths in the final summary. Verify the new file already
  carries the Accure-renamed symbols (e.g. `x-accure-directory`) by diffing against
  pristine upstream.
- if upstream extracts shared policy into a helper, move Accure-specific additions
...
- if upstream deleted a file, analyse whether the Accure behaviour should be
...
**commenting it out with `accurecode_change` markers** over deletion when the
surrounding structure (an `if`, a loop) still makes sense. That keeps the
intent visible to the next merger. Example:

```ts
} else if (input?.scope !== "project" && !Flag.ACCURECODE_EXPERIMENTAL_WORKSPACES) {
...
shows up in `git diff --diff-filter=U`; the new sibling may need a Accure feature
...
- `accurecode_change` comments in the *auto-merge* snapshot that didn't end up in
...
- **Orphaned imports and references.** A rename upstream may leave a Accure
...
- **Partial auto-merges.** Upstream may have refactored a region Accure
  ...
  path, the file compiles upstream but breaks on Accure.
...
- other CI guards that touched files imply (knip for `accure-vscode/`,
  `check-accurecode-change`, source-links, visual regression,
  `script/check-forbidden-strings.ts`)
- if you encounter a hardcoded upstream URL, repo path, or attribution string
  during conflict resolution that obviously shouldn't ship in Accure (e.g. another
...
shared code, but the change cascades into Accure-only files (accurecode tests,
accure-specific source, plugins) that still reference the old symbol. Those
...
### 9.6. Handle upstream-added tests that diverge from Accure

Upstream sometimes adds tests that encode design contracts Accure intentionally
breaks. These auto-merge cleanly and then fail. Three resolution patterns:

- **Rewrite the test** when the test is a contract assertion and Accure has a
  different but equally valid contract. Invert or adjust the assertion with a
  `accurecode_change` marker explaining the divergence.
- **Skip the test** when the test relies on patterns that Accure has replaced
  ...
  Accure's extensions break). Mark with `accurecode_change` and a rationale
  explaining what would need to change for the test to run.
- **Delete the test** when it covers functionality Accure deliberately removed
  (fallback paths, deprecated endpoints, products Accure doesn't ship). Note
...
`packages/extensions/zed/extension.toml` (version field + 5 Accure-Org download
...
(e.g. `1.14.30`), whose release tag does not exist on Accure's pipeline, so the
...
it any time to rebase the version back onto Accure main (useful during
...
- **Non-trivial merge decisions**: a short section per file (or group of
  ...
  Focus on *what Accure behaviour survived* and *what upstream features were
  adopted*. Link to Accure PRs when a `accurecode_change` encodes a specific fix.
...
- Upstream can reintroduce bugs a Accure `accurecode_change` had already fixed —
  during big refactors check every `accurecode_change` the refactor touched.
- "Take-theirs" on an inner conditional is often wrong when Accure added an outer
...
- Stricter DOM lib types (upstream TS upgrade) can surface latent casting
  issues around `WebSocket.send`, `Headers`, etc. — prefer narrowing the Accure
...
- Accure code may rely on ambient context (async-local storage, globally-set
  flags, process env) being populated at a lifecycle moment that upstream
  refactors away. If Accure behaviour reads ambient state during init, forked
...
- Tests that intercept via process-global or module-global spies can become
  ...
  the spy.
- When Accure extends a shared data shape with extra optional fields, different
  ...
  every Accure addition — audit the encoding assumption before adding fields.
...
- Upstream-added tests can encode a design contract Accure deliberately breaks.
  The test passes upstream because upstream doesn't share Accure's requirement.
  Decide between refactoring Accure to match the upstream contract or rewriting
  the test to assert Accure's divergent contract — with a `accurecode_change`
