#!/usr/bin/env bun
/**
 * Transform i18n translation files with Accure branding
 *
 * This script handles i18n files by:
 * 1. Taking upstream's version as the base (to get new translation keys)
 * 2. Applying intelligent string replacements for Accure branding
 * 3. Preserving lines marked with `// accurecode_change`
 *
 * String replacement rules:
 * - opencode.ai -> accure.ai (domain)
 * - app.opencode.ai -> app.accurecode.ai (app domain)
 * - OpenCode -> Accure (product name in user-visible text)
 * - opencode upgrade -> accure upgrade (CLI commands)
 * - npx opencode -> npx accure (CLI invocation)
 * - anomalyco/opencode -> Accure-Inc/accure-code (GitHub repo)
 *
 * Preserved (not replaced):
 * - opencode.json (actual config filename)
 * - .opencode/ (actual directory name)
 * - Lines with `// accurecode_change`
 */
...
/**
 * Check if a line should be preserved (has accurecode_change marker)
 */
function shouldPreserveLine(line: string): boolean {
  return line.includes("// accurecode_change")
}

/**
 * Apply string replacements to content, preserving accurecode_change lines
 */
export function transformI18nContent(
  ...
  for (const line of lines) {
    // Skip lines marked with accurecode_change
 ...
 * Takes upstream version (theirs) and applies Accure branding
 */
export async function transformConflictedI18n(
  ...
  for (const file of files) {
    ...
    if (!options.dryRun && (await oursHasAccurecodeChanges(file))) {
    ...
    // Then apply Accure branding transformations
    ...
    if (options.dryRun) {
      ...
      success(`Transformed ${file}: took upstream + ${result.replacements} Accure branding replacements`)
    }
  }
