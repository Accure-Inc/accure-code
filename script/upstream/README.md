...
Scripts for automating the merge of upstream opencode changes into Accure.
...
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/accure-opencode-v1.1.44
```
...
| `opencode-changesets.ts` | Generate Accure changesets from upstream opencode release notes |
| `fix-accurecode-markers.ts` | Rebuild `accurecode_change` markers for one file against the last merged upstream |
...
| Script | Description |
|---|---|
| `transforms/package-names.ts` | Transform opencode package names to accure |
| `transforms/preserve-versions.ts` | Preserve Accure's package versions |
| `transforms/keep-ours.ts` | Keep Accure's version of specific files |
| `transforms/skip-files.ts` | Skip/remove files that shouldn't exist in Accure |
| `transforms/transform-i18n.ts` | Transform i18n files with Accure branding |
| `transforms/transform-take-theirs.ts` | Take upstream + apply Accure branding for branding-only files |
| `transforms/transform-package-json.ts` | Enhanced package.json with Accure dependency injection and newest-Bun-wins reconciliation |
...
4. **Create branches**
   ...
   - `<author>/accure-opencode-<version>` - Merge target branch
   - `<author>/opencode-<version>` - Transformed upstream branch

5. **Apply ALL transformations to upstream branch (PRE-MERGE)**:
   - Remove files that should not exist in Accure (`skipFiles`)
   - Transform package names (opencode-ai -> @accurecode/cli)
   - Preserve Accure's versions
   - Transform i18n files with Accure branding
   - Transform branding-only files (UI components, configs)
   - Transform package.json files (names, deps, Accure injections)
   - Transform script files (GitHub API references)
   - Transform extension files (Zed, etc.)
   - Transform web/docs files
   - Reset Accure-specific files

6. **Merge** transformed upstream into Accure branch
   - Since all branding transforms are applied pre-merge, conflicts should be minimal
   - Remaining conflicts are files with actual code differences (accurecode_change markers)

7. **Auto-resolve** any remaining conflicts
   - Skip files that shouldn't exist in Accure
   - Keep Accure's version of specific files
...
```typescript
{
  ...
  packageMappings: [
    { from: "opencode-ai", to: "@accurecode/cli" },
    { from: "@opencode-ai/cli", to: "@accurecode/cli" },
    // ...
  ],

  // Files to always keep Accure's version (never take upstream)
  keepOurs: [
    "README.md",
    "CONTRIBUTING.md",
    "AGENTS.md",
  ...
  // Files to take upstream + apply Accure branding transforms
  takeTheirsAndTransform: [
    "packages/ui/src/**/*.tsx",
    // ...
  ],

  // Accure-specific directories (preserved)
  accureDirectories: [
    "packages/opencode/src/accurecode",
...
**Key insight**: By applying all branding transforms to the upstream branch BEFORE merging, we eliminate most conflicts that would otherwise occur due to branding differences (OpenCode -> Accure).

### Transform Order (Pre-Merge)

The following transforms are applied to the opencode branch before merging:

1. **Skip files** - Remove upstream-only packages/files that should not exist in Accure
2. **Package names** - `opencode-ai` -> `@accurecode/cli`, etc.
3. **Versions** - Preserve Accure's version numbers
4. **i18n files** - OpenCode -> Accure in user-visible strings
5. **Branding files** - UI components, configs with branding only
6. **package.json** - Names, dependencies, Accure injections
...
|---|---|---|
| i18n files | `i18n-transform` | Take upstream, apply Accure branding |
| UI components | `take-theirs-transform` | Take upstream, apply branding (no logic changes) |
| package.json | `package-transform` | Take upstream, transform names, inject Accure deps |
...
| README/docs | `keep-ours` | Keep Accure's version |
| GitHub workflows | `keep-ours` | Keep Accure's version (manual review) |
| Code with markers | `manual` | Has `accurecode_change` markers, needs review |
...
- Accure had `Accure` branding
- Git saw these as conflicting changes

Now:

- We transform upstream to `Accure` branding BEFORE merge
- Both branches have the same branding
- Git sees no conflict for branding-only files

The only remaining conflicts are files with **actual code differences** - files with `accurecode_change` markers that contain Accure-specific logic.

### Bun Version Safety

Root `package.json` reconciliation uses the newer valid `packageManager` Bun version from Accure and upstream. An older upstream version cannot downgrade Accure, while a newer upstream version is retained as an upgrade. Before the merge is finalized, `merge.ts` also validates the result against the pristine Accure base and upstream commit and aborts if the merged Bun version is lower than either input.
...
| `.worktrees/opencode-merge/accure-main` | The Accure base branch snapshot used for the merge |
...
- Accure-only paths: anything under `packages/accure-*/`, any `**/accurecode/**` subdir, `script/upstream/`.
- Non-code assets: SVG, PNG, fonts, archives, lock files, etc. (see `SKIP_EXTENSIONS` / `SKIP_FILENAMES` in the script).
- Files covered by the merge config's `keepOurs` or `skipFiles` lists in `utils/config.ts` — these are intentionally preserved or removed in Accure and must not be bulk-reset.
...
| `markers-only` | Stripping `accurecode_change` markers makes local match upstream | reset |
...
| `upstream-missing` | File does not exist upstream (accure-only, intentional) | skipped |
| `local-missing` | File tracked but missing locally (deleted in Accure) | skipped |
...
# Create PR: catrielmuller/accure-opencode-v1.1.44 -> main

# Second merge: v1.1.50 based on the previous PR (without waiting for approval)
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/accure-opencode-v1.1.44

# Create PR: catrielmuller/accure-opencode-v1.1.50 -> catrielmuller/accure-opencode-v1.1.44
# OR: catrielmuller/accure-opencode-v1.1.50 -> main (once first PR is merged)
```
...
bun run analyze.ts --version v1.1.50 --base-branch catrielmuller/accure-opencode-v1.1.44

# 2. Run the merge
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/accure-opencode-v1.1.44

# 3. Create PR from catrielmuller/accure-opencode-v1.1.50
#    - Target: catrielmuller/accure-opencode-v1.1.44 (if first PR not merged yet)
...
2. Look for `accurecode_change` markers to identify Accure-specific code
3. Resolve conflicts, keeping Accure-specific changes
4. Stage and commit:
   ```bash
   git add -A
