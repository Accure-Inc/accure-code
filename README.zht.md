...
Accure Code 是一個 AI 編碼代理，可在你工作的任何地方使用：[VS Code](https://accure.ai/landing/vs-code)、[JetBrains](https://accure.ai/features/jetbrains-native) 和 [CLI](https://accure.ai/cli)。它是開源專案，並採用開放定價。你可以從 500 多個模型中選擇，在任務中途切換模型，並按模型供應商的價格付費，沒有加價。開始使用不需要 API 金鑰。
...
直接安裝 [Accure Code 擴充功能](vscode:extension/accurecode.accure-code)，或從 [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=accurecode.Accure-Code) 取得。建立帳戶後，你可以按供應商價格使用 500 多個模型，包括 GPT-5.5、Claude Opus 4.7、Claude Sonnet 4.6 和 Gemini 3.1 Pro Preview。
...
從 JetBrains Marketplace 安裝 [Accure Code 外掛](https://plugins.jetbrains.com/plugin/28350-accure-code)，或在任何 JetBrains IDE 的 `Settings → Plugins` 中搜尋 "Accure Code"。
...
無需本機電腦，在 Web 上透過 [app.accure.ai/cloud](https://app.accure.ai/cloud) 執行 Accure。
...
在 [app.accure.ai/code-reviews](https://app.accure.ai/code-reviews) 為你的 Pull Request 設定自動 AI 程式碼審查。
...
在 [app.accure.ai/claw](https://app.accure.ai/claw) 啟動你的常駐 AI 代理。
...
從 [Releases 頁面](https://github.com/Accure-Org/accurecode/releases) 下載最新二進位檔。

| 平台 | 資源 |
|---|---|
| Windows（大多數 PC） | `accure-windows-x64.zip` |
...
Accure 內建可依任務切換的專用 Agents。你也可以建立自己的自訂 Agents。
...
深入了解 [agents 和自訂 agents](https://accure.ai/docs/code-with-ai/agents/using-agents)。
...
使用 `--auto` 執行 `accure run`，可在 CI/CD 管線中進行無提示的完全自主操作：
...
關於設定和其他內容，請查看[文件](https://accure.ai/docs)。
...
<details>
<summary>Accure CLI 從何而來？</summary>

Accure CLI 是 [OpenCode](https://github.com/Accure-Org/accurecode) 的 fork，並增強為可在 Accure agentic engineering 平台中使用。

</details>

---

**加入社群** [Discord](https://accure.ai/discord) | [X](https://x.com/accurecode) | [Reddit](https://www.reddit.com/r/accurecode/)
