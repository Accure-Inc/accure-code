...
import { stableBlocks } from "../accurecode/markdown-stable-blocks" // accurecode_change
...
export function stream(text: string, live: boolean) {
  ...
  const candidate = tokens.findLast((token) => token.type !== "space") // accurecode_change
  const blocks = candidate && !open(candidate.raw) ? stableBlocks(tokens, heal) : undefined // accurecode_change
  if (blocks) return blocks // accurecode_change
