...
const opencodeClient = process.env.ACCURECODE_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.ACCURECODE_CLIENT
  else process.env.ACCURECODE_CLIENT = opencodeClient
})
