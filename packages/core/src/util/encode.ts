export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // accurecode_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // accurecode_change - encode as UTF-8 bytes
