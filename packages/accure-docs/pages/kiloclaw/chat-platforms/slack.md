---
title: "Slack"
description: "Using AccureClaw with Slack"
---

# Slack

This page covers everything you need to use AccureClaw with Slack: connecting your bot, controlling who can DM it, and adding it to channels.

## Connecting AccureClaw to Slack

{% youtube url="https://youtu.be/Q5bt-qH-_pY" title="Slack Setup Guide" caption="How to connect your AccureClaw agent to Slack" /%}

Create a Slack app from the OpenClaw manifest and link it to your AccureClaw dashboard.
...
4. Customize the manifest before creating:
   ...
   - Update the slash command if desired (e.g., `/accureclaw`)
5. Click **Create**
...
### Step 3: Connect Slack to AccureClaw

1. In the [AccureClaw UI](https://app.accurecode.ai/claw), find the Slack integration section (may show "not configured")
2. Enter both tokens:
   - The `xapp-` app-level token
   - The `xoxb-` bot user OAuth token
3. Click **Save**
4. Scroll to the top of the AccureClaw UI and click **Redeploy**. Wait for the instance to come back up

### Step 4: Pair Slack with AccureClaw

1. In Slack, DM the app and send any message — this triggers the pairing flow
2. The app will return a pairing code
3. Return to [app.accurecode.ai/claw](https://app.accurecode.ai/claw) and confirm the pairing code and approve
...
By default, AccureClaw can respond to any DMs and will not respond in Slack channels, even if added.

## Making AccureClaw DM-Only (from you)

By default, AccureClaw will respond to DMs from any user in Slack.
...
## Adding AccureClaw to a Slack Channel

By default, AccureClaw will not respond in Slack channels, even if added. To have AccureClaw participate in a Slack channel:
...
Tell your AccureClaw agent (via DM):
