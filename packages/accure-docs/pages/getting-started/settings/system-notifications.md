...
description: "Configure native OS notifications for Accure Code"
platform: legacy
---
...
Accure Code uses system notifications to inform you about:
...
Accure Code's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. Accure Code uses macOS's built-in command to display notifications.
...
**How it works:** Accure Code first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
