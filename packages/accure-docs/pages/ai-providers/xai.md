---
title: "Using xAI Grok with Accure Code"
description: "Connect xAI's Grok models to Accure Code. Use a SuperGrok or X Premium subscription via OAuth or a paid API key. Guide to setup in VS Code and the CLI."
sidebar_label: xAI (Grok)
---

# Using xAI (Grok) With Accure Code
...
Accure Code supports two ways to connect xAI:
...
- **OAuth login — no API keys:** Sign in through your browser and Accure Code handles token management automatically.
- **Automatic token refresh:** Accure Code refreshes your access token in the background so long-running sessions stay authenticated.

{% callout type="note" %}
SuperGrok and X Premium subscription access works with Accure Code's core functionality (VS Code extension and CLI). For cloud features such as Cloud Agents or AccureClaw, use the [Accure Gateway](/docs/gateway) — the Gateway supports xAI via [BYOK](/docs/getting-started/byok) with an API key (OAuth and subscription-based access are not supported through the Gateway).
{% /callout %}
...
5. Back in Accure Code settings, select your desired Grok model.
6. Save.
...
```bash
accure auth login --provider xai
```

Accure Code offers three methods at the prompt:
...
- **Sign out:** To disconnect in VS Code, use the "Disconnect" button in the provider settings. In the CLI, run `accure auth logout` and choose xAI.
...
- **Token rotation:** xAI rotates refresh tokens on each use. Accure Code persists the latest tokens automatically. If you run Accure Code from multiple processes simultaneously, the first refresh can invalidate the other process's token — re-run `accure auth login --provider xai` to restore the session.
...
3. **Create a Key:** Click to create a new API key. Give your key a descriptive name (e.g., "Accure Code").
