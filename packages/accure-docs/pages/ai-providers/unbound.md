---
title: "Using Unbound with Accure Code"
description: "Access Claude, GPT, and other leading models through the Unbound proxy in Accure Code. Setup guide for VS Code and the CLI."
sidebar_label: Unbound
---

# Using Unbound With Accure Code

Accure Code supports accessing models through [Unbound](https://getunbound.ai/), a platform that focuses on providing secure and reliable access to a variety of large language models (LLMs). Unbound acts as a gateway, allowing you to use models from providers like Anthropic and OpenAI without needing to manage multiple API keys and configurations directly. They emphasize security and compliance features for enterprise use.
...
2.  **Create an Application:** Go to the [Connect](https://gateway.getunbound.ai/connect) page and select "Accure Code".
3.  **Copy the API Key:** Copy the API key to your clipboard.

## Supported Models

Unbound allows you configure a list of supported models in your application, and Accure Code will automatically fetch the list of available models from the Unbound API.
...
Unbound is not yet available as a CLI provider. Check the [Accure Code releases](https://github.com/Accure-Inc/accure-code/releases) for updates on provider support.
{% /callout %}
