...
description: "Architecture of the Accure VS Code extension and Agent Manager"
---

# VS Code Extension Architecture

The VS Code extension (`packages/accure-vscode/`) is a client of [Accure CLI runtime](/docs/contributing/architecture/cli-runtime). It bundles platform CLI binary, starts one shared editor-owned `accure serve` server on demand, and drives that server through generated SDK HTTP calls plus global SSE.
...
Activation creates one `AccureConnectionService`. It owns one `ServerManager`, one active SDK client, and one SSE adapter. `ServerManager` owns child process lifecycle. This editor-owned child is separate from detached local daemon managed by `accure daemon`.

```mermaid
flowchart LR
  subgraph host ["VS Code extension host"]
    consumers["Sidebar, tabs, panels, services"]
    service["AccureConnectionService"]
    manager["ServerManager"]
    sdk["Generated SDK client"]
    sse["SdkSSEAdapter"]
  end

  server["bin/accure serve --port 0"]
...
| Binary | Uses extension `bin/accure`, or `bin/accure.exe` on Windows |
| Port | Starts `accure serve --port 0`; CLI server prefers `4096`, then asks OS for free port |
| Authentication | Generates random 32-byte hex password per spawn and passes it as `ACCURECODE_SERVER_PASSWORD`; username defaults to `accure` |
...
| Panels | Settings, profile and marketplace surfaces, sub-agent viewers, Agent Manager, AccureClaw |
| Diff | Diff Viewer, Diff Virtual, and diff source catalog |
| Editor assistance | Autocomplete and commit-message generation |
| Integrations | Browser automation MCP registration and AccureClaw bootstrap |
...
webview vscode.postMessage()
  -> AccureProvider host handler
  -> generated SDK HTTP request
  -> CLI runtime
  -> /global/event SSE
  -> SdkSSEAdapter
  -> AccureConnectionService subscribers
  -> AccureProvider directory/session filtering and stream coalescing
  -> webview postMessage()
```
...
| Backend | Shared `accure serve` process | Same shared process |
...
Agent Manager persists state in `.accurecode/agent-manager.json` and worktrees under `.accurecode/worktrees/`. Startup migration moves Agent Manager-owned data from legacy `.accurecode/` paths when target items do not already exist and repairs git worktree refs.
...
| CLI PTY WebSocket tab | Agent Manager and `accure serve` server | Server-created PTY session streamed over loopback WebSocket |

Agent Manager PTY WebSocket URL uses `auth_token=<base64 accure:password>` query mode because browser WebSocket API cannot attach Basic header. Webview CSP permits loopback HTTP and WebSocket origins for active server port. CLI also exposes scope-bound short-lived PTY ticket API as alternate browser WebSocket auth mode.

## Config split

| Config owner | Examples |
|---|---|
| VS Code settings | `accure-code.new.*` extension UI, proxy, autocomplete, and integration settings |
| CLI config | Global and project `accure.jsonc`, `accure.json`, compatible OpenCode files, provider auth, tools, permissions, modes |
...
| Resource | Behavior |
|---|---|
| CLI executable | Platform binary under extension `bin/`; Windows uses `accure.exe` |
| CLI Tree-sitter WASM | Copied under `bin/tree-sitter`; backend spawn sets `ACCURECODE_TREE_SITTER_WASM_DIR` |
...
| Empty-window indexing | Sets `ACCURECODE_DISABLE_CODEBASE_INDEXING=vscode-no-workspace` so CLI reports indexing disabled |

Speech-to-text captures audio locally, then sends completed recording through shared editor-owned `accure serve` server to authenticated Accure Gateway transcription path. It is batch transcription, not direct provider streaming.
...
| AccureClaw webview | `webview-ui/accureclaw/index.tsx` | `dist/accureclaw.js` |
...
Extension host bundle targets Node/CommonJS. Browser webviews and shared worker use esbuild browser bundles. Run `bun run typecheck`, `bun run lint`, and targeted unit tests from `packages/accure-vscode/` after changing this area.
...
| Concern | Source path |
|---|---|
| Activation | `packages/accure-vscode/src/extension.ts` |
| Editor-owned server child process | `packages/accure-vscode/src/services/cli-backend/server-manager.ts` |
| Shared SDK and SSE ownership | `packages/accure-vscode/src/services/cli-backend/connection-service.ts` |
| SSE reconnect adapter | `packages/accure-vscode/src/services/cli-backend/sdk-sse-adapter.ts` |
| Agent Manager | `packages/accure-vscode/src/agent-manager/` |
| Build entries | `packages/accure-vscode/esbuild.js` |
