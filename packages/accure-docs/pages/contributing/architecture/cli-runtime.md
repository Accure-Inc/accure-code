...
description: "Architecture of the Accure CLI runtime, daemon, server, persistence, SDK, and indexing"
---

# CLI Runtime Architecture

The CLI (`packages/opencode/`) is Accure Code's local agent engine. It owns agent execution, tools, sessions, provider integration, configuration, local persistence, directory routing, and HTTP surfaces used by editor clients and Accure Console.
...
| Term | Meaning |
|---|---|
| Accure CLI runtime | Local agent engine in `packages/opencode/` |
...
| Process-shared state | Runtime service state shared by every directory context in one Accure CLI process |
| Modes | Configurable agent presets for tools, prompts, restrictions, and behavior |
| MCP | Protocol for extending agent tools |

One `accure serve` process can host several local runtime instances. Directory-keyed state stays isolated. Process-shared service state does not.

## Command entry points

| Entry point | Command or caller | Runtime model |
|---|---|---|
| Interactive TUI | `accure` | Attaches to local daemon when available; otherwise starts Bun worker and sends SDK-shaped requests over RPC |
| Headless run | `accure run` | Uses daemon attach when available, then embedded server fetch fallback |
| Attached run | `accure run --attach <url>` | Targets explicit running `accure serve` server |
| Explicit API server | `accure serve` | Starts HTTP + SSE server for external local clients |
| Local daemon | `accure daemon start` | Starts detached `accure serve` child for reuse |
| Browser console | `accure console` | Starts or reuses local daemon and opens daemon-served `/console` UI |
| Editor-spawned server | VS Code or JetBrains client | Starts bundled `accure serve --port 0` child owned by editor client, not local daemon manager |

```mermaid
flowchart LR
  run["accure run"]
  tui["accure TUI"]
  daemon["Detached daemon: accure serve"]
  worker["Bun worker"]
  rpc["RPC-backed fetch and global events"]
  embedded["Embedded Server.Default().app.fetch"]
  serve["Explicit accure serve"]
  editors["VS Code or JetBrains"]
  editorServer["Editor-owned accure serve --port 0"]
...
Each running editor host starts one editor-owned `accure serve` server. That server can handle coding sessions for workspace root and additional worktree directories at same time. It does not start separate server process for each directory.

```mermaid
flowchart LR
  views["Editor views<br/>Workspace root and worktrees"]
  server["One editor-owned<br/>accure serve server"]
  store["InstanceStore"]
...
|---|---|---|
| Local `accure serve` access | HTTP, SSE, and selected WebSocket access to local server | Accure CLI server and spawning local client |
| Outbound provider authentication | Model provider, Accure Gateway, catalog, and indexing access | Accure CLI provider router and auth stores |
| Remote MCP OAuth | Browser authorization and credentials for remote MCP server | Accure CLI MCP runtime |

### Local `accure serve` access

Server Basic Auth is optional. It becomes required when `ACCURECODE_SERVER_PASSWORD` is non-empty. Default username is `accure`; `ACCURECODE_SERVER_USERNAME` can override it.
...
| PTY shell child | Removes `ACCURECODE_SERVER_PASSWORD` and `ACCURECODE_SERVER_USERNAME` from spawned user-shell environment |
...
Provider auth records use `api`, `oauth`, or `wellknown` variants in `${Global.Path.data}/auth.json`, written with mode `0600`. `ACCURECODE_AUTH_CONTENT` can supply process-local auth JSON. Separate v2 multi-account auth store also exists for account-oriented flows.

| Path | Behavior |
|---|---|
| Direct providers | Use provider-specific keys, OAuth records, environment values, and configured endpoints |
| Accure Gateway | Resolves Accure model access and model catalog through gateway client |
| Anonymous Accure | If no Accure key exists, provider loader sets API key value `anonymous`; gateway model catalog can fall back to public unauthenticated endpoint |
| Organization catalog | Accure model fetch includes organization ID when resolved from config, auth, or environment |
...
| Indexing auth | Resolves indexing-specific Accure config first, then provider config, auth record, provider options, and `ACCURECODE_API_KEY` / `ACCURECODE_ORG_ID` environment values |
...
2. `x-accure-directory` request header.
3. Server process cwd.

Local routing workspace selection is separate. Session workspace, `workspace` query parameter, and `ACCURECODE_WORKSPACE_ID` can select workspace context. Configured `ACCURECODE_WORKSPACE_ID` keeps requests local to current workspace runtime. Other selected workspaces resolve through workspace-routing adapter to local directory or remote target.
...
| Tool registry | Loads built-in, Accure-specific, MCP, and readiness-gated semantic search tools |
...
| Provider router | Resolves direct providers, Accure Gateway, custom endpoints, and credentials |
| HTTP server | Publishes REST, WebSocket, and SSE surfaces |

## Daemon lifecycle

`accure daemon start|status|stop|restart` manage a detached local `accure serve` child, with bare `accure daemon` equivalent to `accure daemon start`. `accure console` calls the same start path, so it reuses a healthy daemon instead of spawning a second process, while `accure console stop` aliases `accure daemon stop`.
...
| Child process | Detached `accure serve --hostname <host> --port <port>` process |
...
| Opt-out | `ACCURECODE_NO_DAEMON` disables automatic attach by clients; explicit daemon commands still manage daemon |

Daemon credentials differ from editor-spawned server credentials. Current daemon source stores username `accure`, password `accure`, and base64 Basic token in `daemon.json`. File permissions protect this local credential record. Editor clients generate random passwords per spawned server.
...
| Area | Behavior |
|---|---|
| Default database | `${Global.Path.data}/accure.db` |
| Override | `ACCURECODE_DB`; relative paths resolve under data directory; `:memory:` is accepted |
...
Snapshot implementation state is directory-keyed through `InstanceState`. One `Snapshot.Service` also owns process-shared slow-snapshot guard state outside directory cache. This distinction matters when multiple Agent Manager worktrees use same `accure serve` process.
...
3. Accure-specific API groups and handlers live under `packages/opencode/src/accurecode/server/httpapi/` and enter shared API through narrow injection seams.
4. `packages/sdk/js/script/build.ts` generates TypeScript v2 client from CLI OpenAPI.
5. `packages/sdk/js/src/v2/client.ts` adds `createAccureClient()` wrapper for directory and workspace routing, Electron and Node fetch compatibility, and clearer empty-response errors.
...
| Order | Source |
|---|---|
| 1 | Legacy Accure migrations |
| 2 | Organization modes |
| 3 | Auth-record `.well-known/opencode` remote config |
| 4 | Global config files |
| 5 | Explicit `ACCURECODE_CONFIG` file |
| 6 | Project `accure.json[c]` and `opencode.json[c]` files plus discovered config directories |
| 7 | `ACCURECODE_CONFIG_DIR` directory |
| 8 | `ACCURECODE_CONFIG_CONTENT` |
| 9 | Active Accure Cloud organization config |
...
Global config files load from `${Global.Path.config}`. Project updates prefer existing config files found in ancestor `.accurecode`, `.accurecode`, or `.opencode` directories, then existing project root config files, then create `.accurecode/accure.json`. Global indexing settings can carry provider and storage defaults, but global `indexing.enabled` is stripped so project enablement remains local in effective instance config.
...
Runtime config loading is separate from editor-facing JSON Schema publication. Cloud-served schema improves validation and completion for `accure.json` and `accure.jsonc`; it does not load, apply, or override effective runtime config. When adding or changing config key, follow [CLI Config Schema](/docs/contributing/architecture/config-schema) so CLI source and cloud overlay stay aligned.
...
`accure console` starts or reuses daemon, opens `/console`, and prints Console launch URL. Browser launch URL embeds daemon Basic credentials so initial request authenticates.

| Area | Behavior |
|---|---|
| Frontend | Solid/Vite app in `packages/accure-console/` |
...
| SDK | Console calls generated JavaScript SDK through `createAccureClient()` |
...
`packages/accure-indexing/` owns indexing engine. CLI bridge injects indexing plugin by default unless default plugins are disabled, then starts indexing asynchronously per normalized directory during instance bootstrap.

| Area | Behavior |
|---|---|
| Bootstrap | `AccurecodeBootstrap` forks indexing initialization so instance startup is not blocked |
...
| Worktrees | Agent Manager `.accurecode/worktrees/` and legacy `.accurecode/worktrees/` paths return disabled status |
| Empty VS Code window | Extension sets `ACCURECODE_DISABLE_CODEBASE_INDEXING=vscode-no-workspace`; bridge reports disabled status |
| Embeddings | Supports Accure, OpenAI, Ollama, OpenAI-compatible, Gemini, Mistral, Vercel AI Gateway, Bedrock, OpenRouter, and Voyage configuration |
...
| Daemon | `packages/opencode/src/accurecode/daemon/` |
...
| Snapshots | `packages/opencode/src/snapshot/index.ts`{% linebreak /%}`packages/opencode/src/accurecode/snapshot/track.ts` |
| SDK | `packages/sdk/js/`{% linebreak /%}`script/generate.ts` |
| Console | `packages/accure-console/`{% linebreak /%}`packages/opencode/src/accurecode/console/` |
| Indexing | `packages/accure-indexing/`{% linebreak /%}`packages/opencode/src/accurecode/indexing.ts` |
