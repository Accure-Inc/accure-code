---
title: "Migration"
description: "Migrate your team to Accure Code Enterprise"
---

# Migration

Switch to **Accure Teams** or **Accure Enterprise** from other AI coding tools and experience transparent pricing, no vendor lock-in, and superior team management capabilities.

## Why Teams Switch to Accure
...
**Accure Teams** and **Accure Enterprise** show you exactly what each AI request costs - no markup, no hidden fees, complete transparency.

### No Rate Limiting

**Other tools** slow you down with rate limits and model switching when you need AI most.

**Accure Teams** and **Accure Enterprise** never limit your usage - pay for what you use, use what you need.

### True Team Management

**Other solutions** offer basic user management with limited visibility.

**Accure Teams** provides comprehensive team analytics, role-based permissions, and detailed usage insights, while **Accure Enterprise** adds advanced governance, audit logging, and enterprise-level security controls.
...
### What You Gain with Accure Teams or Accure Enterprise
...
3. **Calculate current costs** (if visible) vs. Accure pricing
4. **Plan migration timeline** to minimize disruption

**Step 2: Accure Setup**

1. **Create organization** at [app.accurecode.ai](https://app.accurecode.ai)
2. **Subscribe to Teams ($15/user/month)** or **Enterprise ([Contact Sales](https://accure.ai/contact-sales))**
3. **Configure team settings** and usage policies
4. **Purchase initial AI credits** based on usage estimates

**Step 3: Team Migration**

1. **Invite team members** to Accure
2. **Install Accure Code extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on Accure Code features and workflows

**Step 4: Full Transition**

1. **Monitor usage patterns** in Accure dashboard
...
| Cursor Feature | Accure Equivalent |
|---|---|
...
2. **Install Accure Code** extension for team members
3. **Start with simple tasks** in Accure Code
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use Accure Code** for new projects
...
Accure Code uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.

**Quick Overview:**

- **Project rules**: `.cursor/rules/*.mdc` → `.accurecode/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.accurecode/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in Accure Code (no conversion needed)
- **Global rules**: Recreate in `~/.accurecode/rules/*.md` directory

Accure Code also supports mode-specific rules (`.accurecode/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
- Accure Code extension installation and setup
...
Ready to make the switch? Contact our migration team at migrations@accure.ai to plan your transition to transparent AI coding.
