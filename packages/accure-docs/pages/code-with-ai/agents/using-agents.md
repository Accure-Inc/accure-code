---
title: "Using Agents"
description: "Understanding and using different agents in Accure Code"
---

# Using Agents

Agents in Accure Code are specialized personas that tailor the assistant's behavior to your current task. Each agent offers different capabilities, expertise, and access levels to help you accomplish specific goals.
...
{% youtube url="https://youtu.be/cS4vQfX528w" caption="Explaining the different modes in Accure Code" /%}
...
4. **Accept suggestions:** Click on mode switch suggestions that Accure Code offers when appropriate

   {% image src="/docs/img/modes/modes-2.png" alt="Accepting a mode switch suggestion from Accure Code" width="400" /%}
...
| **Tool Access** | Read-only tools plus restricted file editing (plan files in `.accurecode/plans/` only) |
...
{% callout type="tip" %}
**Keep debugging separate from main tasks:** When using Debug mode, ask Accure to "start a new task in Debug mode with all of the necessary context needed to figure out X" so that the debugging process uses its own context window and doesn't pollute the main task.
{% /callout %}
