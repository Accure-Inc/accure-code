...
|---|---|---|
| `accure-auto/frontier` | Maximum capability with the best available models | Paid |
| `accure-auto/balanced` | Strong performance at a lower cost | Paid |
| `accure-auto/efficient` | Lowest cost per task, with capability matched to difficulty | Paid |
| `accure-auto/free` | The best free models available | Free |

## How It Works

1. Select an Auto Model tier (e.g. `accure-auto/frontier`) in the model dropdown
...
You can see which underlying models are used, as well as the cost, in the expanded model picker. Model mapping information is also available on the [Gateway Model page](/docs/gateway/models-and-providers#accure-autofrontier).
...
1. Accure observes your coding session in context
...
Select an Auto Model tier from the model dropdown in the Accure Code chat interface. That's all you need to do.
{% /callout %}

1. Open Accure Code in VS Code or JetBrains
2. Click the model selector dropdown
3. Choose an Auto Model such as `accure-auto/frontier` or `accure-auto/balanced`
...
- **New Accure Code users** - Great defaults without needing to understand model differences
...
Auto Model is actively being improved. We'd love to hear how it's working for you! Share feedback in our [Discord](https://accure.ai/discord) or [open an issue on GitHub](https://github.com/Accure-Inc/accure-code/issues).
{% /callout %}

## Related

- [Model Selection Guide](/docs/code-with-ai/agents/model-selection) - General guidance on choosing models
- [Using Agents](/docs/code-with-ai/agents/using-agents) - Learn about different Accure Code agents
- [Using Accure for Free](/docs/getting-started/using-accure-for-free) - Cost-effective alternatives
