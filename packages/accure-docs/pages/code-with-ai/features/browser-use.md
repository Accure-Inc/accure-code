---
title: "Browser Use"
description: "Using Accure Code to interact with web browsers"
---

# Browser Use

Accure Code provides browser automation capabilities that let you interact with websites directly from your coding workflow. This feature supports testing web applications, automating browser tasks, and capturing screenshots without leaving your editor.
...
Browser automation is built into the extension and requires no manual setup. Enable it from **Settings → Browser** and Accure handles the rest automatically.

{% /tab %}
{% tab label="CLI" %}

Accure Code uses [Playwright](https://playwright.dev/) for browser automation. Add it to your `accure.jsonc` configuration:
...
By default, Accure Code uses a built-in browser that:

- Launches automatically when you ask Accure to visit a website
- Captures screenshots of web pages
- Allows Accure to interact with web elements
...
1. Ask Accure to visit a website
2. Accure launches the browser and shows you a screenshot
3. Request additional actions (clicking, typing, scrolling)
4. Accure closes the browser when finished

For example:

- `Open the browser and view our site.`
- `Can you check if my website at https://accurecode.ai is displaying correctly?`
...
Accure launches a browser automatically when asked and returns screenshots after each action so you can see what's happening. It can navigate to URLs, click elements, fill in forms, scroll, hover, select from dropdowns, and drag and drop — all driven by natural language instructions in chat.
...
Browser automation is configured in your `accure.jsonc` file. No additional settings are required — Playwright manages the browser lifecycle automatically.
...
To change Browser / Computer Use settings in Accure:

1. Click the gear icon {% codicon name="gear" /%} in Accure Code
2. Open `Browser / Computer Use`

### Enable/Disable Browser Use

**Purpose**: Master toggle that enables Accure to interact with websites using a Puppeteer-controlled browser.
...
**Purpose**: Determines the resolution of the browser session Accure Code uses.
...
**Purpose**: Connect Accure to an existing Chrome browser instead of using the built-in browser.
...
Connect to a visible Chrome window to observe Accure's interactions in real-time:
