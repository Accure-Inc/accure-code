...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in Accure Code. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.
...
- A [Accure Code account](https://app.accurecode.ai) (free to create)
...
## Step 4: Add Your Key via BYOK in Accure

1. Log into the [Accure platform](https://app.accurecode.ai).
2. Navigate to the [Bring Your Own Key (BYOK) page](https://app.accurecode.ai/byok), available in the sidebar under **Account**.
...
Once your BYOK key is saved, Accure Code's autocomplete will automatically use your Codestral key through the Accure Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the Accure Code extension installed.
...
The autocomplete status bar in VS Code shows the current provider ("Accure Gateway") and tracks cumulative cost. With BYOK, requests are billed directly by Mistral at their rates (Codestral has a free tier) and show as $0.00 on your Accure balance.
...
```
Your Editor → Accure Gateway (with your key) → Mistral
```

- The Accure Gateway detects your BYOK key and routes autocomplete requests using it.
- You are billed directly by Mistral — Accure does not add any markup.
- If your BYOK key is invalid, the request will fail (it does not fall back to Accure's keys).

## Troubleshooting

- **Autocomplete not appearing?** Check that autocomplete is enabled in Accure Code settings (it is on by default). Also verify you are signed into Accure Code in the extension.
...
- **Seeing charges on your Accure balance?** If you haven't configured BYOK, autocomplete defaults to using your Accure credits. Add your Codestral key via BYOK to route requests through your own Mistral account.
...
{% youtube url="https://www.youtube.com/embed/0aqBbB8fPho" caption="Setting up Mistral for free autocomplete in Accure Code" /%}

## Step 1: Open Accure Code Settings

In VS Code, open the Accure Code panel and click the **Settings** icon (gear) in the top-right corner.

![Open Accure Code Settings](/docs/img/mistral-setup/01-open-accure-code-settings.png)
...
{% callout type="note" %}
When creating an autocomplete profile, you don't need to select a specific model—Accure Code will automatically use the appropriate Codestral model optimized for code completions.
{% /callout %}
...
## Step 9: Paste API Key in Accure Code

Return to Accure Code settings and paste your API key into the **Mistral API Key** field.
