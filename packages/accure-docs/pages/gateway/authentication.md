...
description: "Learn how to authenticate with the Accure AI Gateway using API keys, session tokens, and Bring Your Own Key (BYOK)."
---

# Authentication

The Accure AI Gateway supports multiple authentication methods depending on your use case.
...
API keys are JWT tokens tied to your Accure account. See [how to get your API key](/docs/getting-started/setup-authentication#accure-gateway-api-key) for step-by-step instructions.
...
```
X-AccureCode-OrganizationId: your_org_id
```
...
BYOK lets you use your own provider API keys with the Accure AI Gateway. When a BYOK key is configured, requests are sent to the provider using your key. You are billed directly by the provider -- Accure does not add any markup.
...
1. Add your provider API key in the [Accure dashboard](https://app.accurecode.ai) or through your Accure Code extension settings
...
4. Usage is tracked but not billed to your Accure balance (cost is set to $0)
5. If your BYOK key fails, the request will not automatically fall back to Accure's keys
...
| `X-AccureCode-OrganizationId` | No | Organization context for org-scoped requests |
| `X-AccureCode-TaskId` | No | Task identifier for prompt cache keying |
| `X-AccureCode-Version` | No | Client version string |
| `x-accurecode-mode` | No | Mode hint for `accure-auto` model routing |
