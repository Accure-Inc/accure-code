...
Accure Code uses tools to interact with your code and environment. These specialized helpers perform specific actions like reading files, making edits, running commands, or searching your codebase. Tools provide automation for common development tasks without requiring manual execution.

## Tool Workflow

Describe what you want to accomplish in natural language, and Accure Code will:
...
**Accure Code:** (Proposes the `write` tool)
...
**Accure Code:** (Proposes the `write_to_file` tool as shown in the image above)
...
**Accure Code:** (Confirms file creation)
...
Every tool use is subject to a permission check. The default action for any tool with no matching rule in your config is **`ask`** — meaning Accure will pause and prompt you before executing it.
...
Every tool use requires your explicit approval. When Accure proposes a tool, you'll see:
...
| `websearch` | Searches the web (Accure/OpenRouter users) | Web |
