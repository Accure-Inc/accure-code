---
title: Write to File Tool | Accure Code Docs
...
- When Accure Code needs to create a new file from scratch
- When Accure Code needs to completely rewrite an existing file
...
- Access Control: Validates against `.accurecodeignore` restrictions before making changes
...
1. **Parameter Validation**: Validates the required parameters and permissions
   ...
   - Validates the file is allowed (not restricted by `.accurecodeignore`)
...
- When creating a new project, Accure Code generates multiple files but lets you review each before committing changes.
- When setting up configuration files, Accure Code shows the proposed configuration in a diff view for approval.
- When generating documentation, Accure Code creates markdown files but lets you make final adjustments in the diff view.
- When developing a prototype, Accure Code shows complete source files in a diff view where you can fine-tune before saving.
