---
title: List Files Tool | Accure Code Docs
description: Learn how the list_files tool displays files and directories within a specified location to help Accure Code understand your project structure.
---

# list_files

The `list_files` tool displays the files and directories within a specified location. It helps Accure Code understand your project structure and navigate your codebase effectively.
...
- When Accure Code needs to understand your project structure
- When Accure Code explores what files are available before reading specific ones
- When Accure Code maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When Accure Code needs to check for specific file types (like configuration files) across a project
...
- Marks files ignored by `.accurecodeignore` with a lock symbol (🔒) when `showAccureCodeIgnoredFiles` is enabled
...
5. **Result Filtering**:
   ...
   - Respects `.gitignore` rules when in recursive mode
   - Handles `.accurecodeignore` patterns, either hiding files or marking them with a lock symbol
6. **Formatting**:
   - Marks directories with a trailing slash (`/`)
   - Sorts results to show directories before their contents for logical hierarchy
   - Marks ignored files with a lock symbol (🔒) when `showAccureCodeIgnored` is enabled
...
- Files ignored by `.accurecodeignore` are marked with a lock symbol (🔒) when `showAccureCodeIgnored` is enabled
...
When `.accurecodeignore` files are used and `showAccureCodeIgnored` is enabled:
...
- When starting a new task, Accure Code may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), Accure Code first lists directories to know where to look.
- When providing recommendations for code organization, Accure Code examines the current project structure first.
- When setting up a new feature, Accure Code lists related directories to understand the project conventions.
