---
title: Search Files Tool | Accure Code Docs
...
The `search_files` tool performs regex searches across multiple files in your project. It helps Accure Code locate specific code patterns, text, or other content throughout your codebase with contextual results.
...
- When Accure Code needs to find where specific functions or variables are used
- When Accure Code helps with refactoring and needs to understand usage patterns
- When Accure Code needs to locate all instances of a particular code pattern
- When Accure Code searches for text across multiple files with filtering capabilities
...
- When asked to refactor a function, Accure Code first searches for all places the function is used to ensure comprehensive changes.
- When investigating bugs, Accure Code searches for similar patterns to identify related issues across the codebase.
- When addressing technical debt, Accure Code locates all TODO comments across the project.
- When analyzing dependencies, Accure Code finds all imports of a particular module.
