---
title: Ask Followup Question Tool | Accure Code Docs
description: Learn how the ask_followup_question tool enables Accure Code to gather additional information needed to complete tasks through interactive communication.
---
...
This tool creates a conversational interface between Accure Code and the user, allowing for gathering clarification, additional details, or user preferences when facing ambiguities or decision points. Each question can include suggested responses to streamline the interaction.

## When is it used?

- When critical information is missing from the original request
- When Accure Code needs to choose between multiple valid implementation approaches
- When technical details or preferences are required to proceed
- When Accure Code encounters ambiguities that need resolution
...
1. **Information Gap Recognition**: Accure Code identifies missing information needed to proceed
2. **Specific Question Creation**: Accure Code formulates a clear, targeted question
3. **Suggestion Development**: Accure Code creates relevant suggested answers (optional but recommended)
...
10. **Task Continuation**: Accure Code proceeds with the task using the new information

## Examples When Used

- When developing a web application, Accure Code might ask about preferred styling frameworks (Bootstrap, Tailwind, custom CSS)
- When creating an API, Accure Code might ask about authentication methods (JWT, OAuth, API keys)
- When refactoring code, Accure Code might ask about prioritizing performance vs. readability
- When setting up a database, Accure Code might ask about specific schema design preferences
- When creating a custom feature, Accure Code might ask about specific behavior expectations
- When troubleshooting errors, Accure Code might ask about specific environment details
