...
description: "How to configure and use MCP servers in the Accure CLI"
---

# Using MCP in the CLI

The Accure CLI supports both local and remote MCP servers. Once added, MCP tools are automatically available to the LLM alongside built-in tools.
...
|---|---|---|
| **Global** | `~/.config/accure/accure.json` | `accure.jsonc`, `config.json` |
| **Project** | `./accure.json` or `./.accurecode/accure.json` | `accure.jsonc` |
...
Browse community-contributed MCP server configurations and agent skills in the [Accure Marketplace](https://github.com/Accure-Org/accure-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
