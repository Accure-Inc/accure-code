...
description: "Understanding MCP server transport options"
---

# MCP Server Transports: STDIO & SSE

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Accure Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Accure Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Accure Code
...
- Single-client scenarios (one Accure Code instance per server)
...
1. The client (Accure Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Accure Code
...
STDIO servers run locally on the same machine as Accure Code, which has several important implications:
...
- **Execution**: Starts and stops with Accure Code (child process lifecycle)
...
- Start when needed by Accure Code
- Not require network configuration
- Need to be installed alongside Accure Code or via a package manager
...
## Configuring Transports in Accure Code

For detailed information on configuring STDIO and SSE transports in Accure Code, including example configurations, see the [Understanding Transport Types](using-in-accure-code#understanding-transport-types) section in the Using MCP in Accure Code guide.
