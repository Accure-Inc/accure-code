...
Auto-Launch Configuration allows you to automatically start a Accure Code task when opening a workspace, with support for specific profiles and modes. This was originally developed as an internal test feature, but we decided to expose it to users in case anyone finds it useful!
...
When you open a workspace in VS Code, Accure Code automatically checks for a launch configuration JSON file. If found, it:
...
1. Create a `.accurecode` directory in your workspace root (if it doesn't exist)
2. Create a `launchConfig.json` file inside the `.accurecode` directory
...
- **`mode`** (string): The Accure Code mode to use for this task. Available modes:
...
- Auto-launch triggers approximately 500ms after Accure Code extension activation
...
1. Verify file location: `.accurecode/launchConfig.json` in workspace root
