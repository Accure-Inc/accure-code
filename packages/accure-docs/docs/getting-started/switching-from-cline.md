...
# Migrating from Cline to Accure

A practical guide for developers switching from Cline to Accure.

## Why Switch?

**Speed.** The Accure Platform is designed to reduce friction across the entire development workflow. Beyond feature parity with Cline, Accure includes Code Reviews, App Builder, Cloud Agents, and one-click Deploy.

**Multi-platform.** Work in VS Code, JetBrains IDEs, CLI, or the web. Your Sessions sync across all of them automatically.

**Specialized modes.** Instead of one agent doing everything, Accure has five specialized modes optimized for different parts of your workflow.

**500+ models.** More providers, more options, better pricing flexibility. Switch models mid-conversation if you want.

---

## Understanding Accure's Mode System

Cline uses a single agent with a Plan/Act toggle. Accure takes a different approach: five specialized modes, each optimized for a specific part of development. You select modes from a dropdown in the interface in the Accure Extension, or with slash commands in the CLI.
...
**Accure:** Select **Ask** from the dropdown
...
**Why this matters:** In Cline, you might accidentally make changes while exploring. In Accure, Ask and Architect modes can't write files, so you're safe to explore without worry. Accure's **Orchestrator** mode lets you intelligently generate and execute subtasks that are optimized for each mode.
...
2. Search "Accure Code"
3. Click Install
4. Find the Accure icon in your sidebar
...
4. Find Accure icon in right sidebar
...
Visit [app.accurecode.ai](https://app.accurecode.ai/) and sign in. This gives you access to:

- **Cloud Agents:** Run Accure without a local machine
- **App Builder:** Build and preview apps directly in your browser
- **Accure Deploy:** One-click deployments
...
1. Click "Try Accure Code for Free" in the Accure panel
2. Sign in with OAuth at accure.ai
3. You'll be redirected back to your IDE

### Configure your provider

**Option 1: Use Accure Gateway (easiest)**

1. Open settings (⚙️ icon)
2. Select "Accure Gateway" as provider
...
## Beyond the IDE: Accure's Platform Features

One of the biggest differences from Cline is that Accure isn't just an IDE extension. It's a platform with multiple interfaces that can all share your Sessions and context.

### Cloud Agents

Run Accure from [app.accurecode.ai/cloud](https://app.accurecode.ai/cloud) without needing your local machine. Great for:
...
Build live apps and sites directly from the web with a real-time preview. Similar to Lovable, but integrated with your Accure Sessions. Deploy with one click when you're ready.

### Accure Deploy

One-click deployments from directly within Accure. Go from code to production without leaving your workflow.
...
Semantic search across your repositories using configurable embedding providers and vector stores. Accure indexes your codebase to deliver more relevant, context-aware responses.
...
| Cline Feature | Accure Equivalent | Notes |
|---|---|---|
...
- **One-click Deploy:** Ship directly from Accure
- **Code Reviews:** AI-powered PR analysis
...
**Q: Can I use both Cline and Accure side-by-side?** Yes. They're separate extensions.
...
1. Install Accure in your primary IDE
...
3. Try Cloud Agents at [app.accurecode.ai](https://app.accurecode.ai/)
4. Install the Accure CLI with `npm install -g @accurecode/cli`
