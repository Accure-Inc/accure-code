...
/**
 * Accure Gateway Authentication Plugin
 *
 * Provides device authorization flow for Accure Gateway
 ...
 */
export const AccureAuthPlugin: Plugin = async (ctx) => {
  return {
    auth: {
      provider: "accure",
      async loader(getAuth, providerInfo) {
        ...
        if (auth.type === "api") {
          return {
            accurecodeToken: auth.key,
          }
        }

        // For OAuth auth, access token contains the Accure token
      ...
      methods: [
        {
          type: "oauth",
          label: "Accure Gateway (Device Authorization)",
          async authorize() {
...
export default AccureAuthPlugin
