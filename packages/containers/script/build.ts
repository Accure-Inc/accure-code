...
const reg = process.env.REGISTRY ?? "ghcr.io/accure-org" // accurecode_change
...
const images = ["base", "bun-node", "jetbrains", "rust", "tauri-linux", "publish"] // accurecode_change

const setup = async () => {
  if (!push) return
  const list = await $`docker buildx ls`.text()
  // accurecode_change start
  if (list.includes("accure")) {
    await $`docker buildx use accure`
    return
  }
  await $`docker buildx create --name accure --use`
