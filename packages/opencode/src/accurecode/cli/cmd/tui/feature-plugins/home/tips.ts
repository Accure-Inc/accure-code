...
export const ACCURECODE_TIPS: Tip[] = [
  ...
  "Create {highlight}accure.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/accure/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.accurecode/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.accurecode/agent/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.accurecode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.accurecode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Accure from reading sensitive files",
  "Use {highlight}accure run{/highlight} for non-interactive scripting",
  "Use {highlight}accure --continue{/highlight} to resume the last session",
  "Use {highlight}accure run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}accure serve{/highlight} for headless API access to Accure",
  "Use {highlight}accure run --attach{/highlight} to connect to a running server",
  "Run {highlight}accure upgrade{/highlight} to update to the latest version",
  "Run {highlight}accure auth list{/highlight} to see all configured providers",
  "Run {highlight}accure agent create{/highlight} for guided agent creation",
  "Run {highlight}accure github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/accure fix this{/highlight} on issues to auto-create PRs",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.accurecode/themes/{/highlight} directory",
  ...
  "Run {highlight}accure debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/accure-org/accurecode{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Accure Gateway for curated, tested models",
