...
import { resolveAccureIndexingAuth } from "@/accurecode/indexing-auth"
...
export async function getAuthOrgId(): Promise<OrgState> {
  try {
    const [cfg, info] = await Promise.all([
      ...
      auth.runPromise((svc) => svc.get("accure")),
    ])
    const id = resolveAccureIndexingAuth({ config: cfg, auth: info }).organizationId
