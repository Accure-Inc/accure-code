...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [`This will allow ${request.permission} until Accure is restarted.`] // accurecode_change
  }

  return [
    "This will allow the following patterns until Accure is restarted.", // accurecode_change
