...
import { existsSync } from "node:fs" // accurecode_change

const subcommand = "pr" // accurecode_change

// accurecode_change start - resolve the currently running CLI instead of hardcoding opencode
export function cliCommand(
  ...
  if (script === subcommand) return [input.execPath] // accurecode_change
...
export const PrCommand = effectCmd({
  command: `${subcommand} <number>`, // accurecode_change
  describe: "fetch and checkout a GitHub PR branch, then run accure", // accurecode_change
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    const cli = cliCommand() // accurecode_change
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        const sessionMatch = prInfo.body.match(/https:\/\/app\.accurecode\.ai\/s\/([a-zA-Z0-9_-]+)/) // accurecode_change
        if (sessionMatch) {
    ...
    UI.println()
    UI.println("Starting accure...") // accurecode_change
    UI.println()

    const run = sessionId ? [...cli, "-s", sessionId] : cli // accurecode_change
    ...
    if (code !== 0) return yield* Effect.die(new Error(`accure exited with code ${code}`)) // accurecode_change
  }),
})
