...
import { serverUrls } from "@/accurecode/cli/server-urls" // accurecode_change
...
import * as AccureListener from "@/accurecode/server/listener" // accurecode_change
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  ...
  return {
    ...
    port: listener.port,
    url: listener.url,
    urls: listener.urls, // accurecode_change
...
const listenEffect: (opts: ListenOptions) => Effect.Effect<EffectListener, unknown> = Effect.fn("Server.listen")(
  function* (opts: ListenOptions) {
    ...
    return {
      ...
      port: address.port,
      url: listenerUrl,
      urls: serverUrls(opts.hostname, address.port), // accurecode_change
...
function startListener(opts: ListenOptions, port: number) {
  ...
  return AccureListener.build(listenerLayer(opts, port), scope).pipe(
    // accurecode_change
