...
import * as AccureOAuthCallback from "../accurecode/mcp-oauth-callback" // accurecode_change
...
<head>
  ...
  <title>Accure - Authorization Successful</title>
...
<head>
  <!-- accurecode_change start -->
  <title>Accure - Authorization Failed</title>
  <!-- accurecode_change end -->
  <style>
    body { font-family: system-ui, -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #eee; }
    .container { text-align: center; padding: 2rem; }
...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  // accurecode_change start - delegate Accure-specific callback binding from here because OAuth state lives in this module
  await AccureOAuthCallback.ensureRunning({
    redirectUri,
  ...
  await new Promise<void>((resolve, reject) => {
    // accurecode_change start - EADDRINUSE can still fire when another process
    ...
    server!.listen(currentPort, () => {
      server!.off("error", onError) // accurecode_change
