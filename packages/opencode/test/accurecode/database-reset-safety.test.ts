...
const paths = [/\bDatabase\.getPath\s*\(/, /\bDatabase\.Path\b/, /accure\.db(?:-wal|-shm)?/]
...
//    so ACCURECODE_DB was not set to :memory:.
// 2. The test process inherited ACCURECODE_DISABLE_CHANNEL_DB=true from the VS Code
//    extension. Database.getPath() therefore returned the real shared database
//    at ~/.local/share/accure/accure.db.
// 3. The upstream resetDatabase() helper deleted accure.db, accure.db-wal, and
//    accure.db-shm. The test then recreated the database and inserted its
//    identifiable "parent" and "child" sessions.
// 4. Conversations stored only in the deleted database were lost. Git
//    worktrees, branches, and .accurecode/agent-manager.json survived because they are
//    stored separately.
//
...
describe("test database safety", () => {
  ...
  test("preserves the resolved database when the in-memory preload is missing", async () => {
    ...
    // user database. This simulates a disk-backed ACCURECODE_DB and verifies that the
    // reset helper rejects it before cleanup can run.
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "sessions.db")
    const previous = Flag.ACCURECODE_DB
    await Bun.write(file, "preserve me")
    Flag.ACCURECODE_DB = file
