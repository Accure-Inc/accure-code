    ...
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<AccureSession.CloseReason>()
          const unsub = yield* bus.subscribeCallback(AccureSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(AccureSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("AccureSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = AccureSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: AccureSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = AccureSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: AccureSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
