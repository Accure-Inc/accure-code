...
import { ACCURECODE_ERROR_CODES, isAccureError, parseAccureErrorCode } from "../../src/accurecode/accure-errors"
...
describe("parseAccureErrorCode", () => {
  ...
  it("extracts PROMOTION_MODEL_LIMIT_REACHED from { code } (top-level)", () => {
    ...
    expect(parseAccureErrorCode(error)).toBe("PROMOTION_MODEL_LIMIT_REACHED")
  })
  ...
  it("returns undefined for non-Accure error codes", () => {
    const error = makeAPIError({
      statusCode: 429,
      responseBody: JSON.stringify({ error: { code: "SOME_OTHER_ERROR" } }),
    })
    expect(parseAccureErrorCode(error)).toBeUndefined()
  })
...
describe("isAccureError", () => {
  it("returns true for PAID_MODEL_AUTH_REQUIRED", () => {
    const error = makeAPIError({
      statusCode: 401,
      responseBody: JSON.stringify({ error: { code: "PAID_MODEL_AUTH_REQUIRED" } }),
    })
    expect(isAccureError(error)).toBe(true)
  })
  ...
  it("returns false for regular 429 errors without Accure code", () => {
    const error = makeAPIError({
      statusCode: 429,
      isRetryable: true,
      message: "Too Many Requests",
    })
    expect(isAccureError(error)).toBe(false)
  })
...
describe("SessionRetry.retryable with Accure errors", () => {
