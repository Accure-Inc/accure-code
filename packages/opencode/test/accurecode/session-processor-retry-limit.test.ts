...
process.env.ACCURECODE_SESSION_RETRY_LIMIT = "2"
...
afterEach(() => {
  delete process.env.ACCURECODE_SESSION_RETRY_LIMIT
})
...
describe("session processor retry limit", () => {
  ...
  it.effect("only positive integers enable the limit", () =>
    Effect.promise(async () => {
      ...
      delete process.env.ACCURECODE_SESSION_RETRY_LIMIT
      expect(Flag.ACCURECODE_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.ACCURECODE_SESSION_RETRY_LIMIT = "0"
      expect(Flag.ACCURECODE_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.ACCURECODE_SESSION_RETRY_LIMIT = "-1"
      expect(Flag.ACCURECODE_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.ACCURECODE_SESSION_RETRY_LIMIT = "abc"
      expect(Flag.ACCURECODE_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.ACCURECODE_SESSION_RETRY_LIMIT = "2"
      expect(Flag.ACCURECODE_SESSION_RETRY_LIMIT).toBe(2)
    }),
  )
  ...
  it.effect("reads env at access time (dynamic getter)", () =>
    Effect.promise(async () => {
      ...
      delete process.env.ACCURECODE_SESSION_RETRY_LIMIT
      expect(Flag.ACCURECODE_SESSION_RETRY_LIMIT).toBeUndefined()
      process.env.ACCURECODE_SESSION_RETRY_LIMIT = "5"
      expect(Flag.ACCURECODE_SESSION_RETRY_LIMIT).toBe(5)
      delete process.env.ACCURECODE_SESSION_RETRY_LIMIT
    }),
  )
})
