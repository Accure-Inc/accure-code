...
import type { TuiAttentionSoundPack } from "@accurecode/plugin/tui"
...
`,
  ...
  process.env.ACCURECODE_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.ACCURECODE_PLUGIN_META_FILE!,
    )
