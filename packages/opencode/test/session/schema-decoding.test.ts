  ...
  test("round-trips every optional field", () => {
    const input = {
      ...
      summary: {
        ...
        deletions: 5,
        files: 2,
        diffs: [{ additions: 1, deletions: 0, file: "a.ts" }], // accurecode_change
      },
  ...
  test("retry carries attempt/message/action/next", () => {
    const input = {
      ...
      action: {
        ...
        message: "Subscribe to OpenCode Go.", // accurecode_change
        label: "subscribe",
        link: "https://opencode.ai/go", // accurecode_change
      },
      next: 500,
    }
    expect(decode(input)).toEqual(input)
