...
class ConnectionDelayTest : SessionControllerTestBase() {
    ...
    fun `test short connecting state does not fire connection banner`() {
        ...
        appRpc.state.value = AccureAppStateDto(AccureAppStatusDto.CONNECTING)
        pause(25)
        appRpc.state.value = AccureAppStateDto(AccureAppStatusDto.READY)
        pause(150)
    ...
    fun `test changed error restarts delay and shows latest state`() {
        ...
        appRpc.state.value = AccureAppStateDto(AccureAppStatusDto.ERROR, error = "first")
        pause(50)
        appRpc.state.value = AccureAppStateDto(AccureAppStatusDto.ERROR, error = "second")
        pause(150)
    ...
    fun `test persistent workspace error is delayed`() {
        ...
        projectRpc.state.value = AccureWorkspaceStateDto(
    ...
    fun `test config warning remains immediate`() {
        ...
        assertEquals(".accurecode/accure.json: Invalid JSON", event.detail)
    }
