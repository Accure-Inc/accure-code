// accurecode_change start - Tests for ACCURECODE_CONFIG_CONTENT merging
import { describe, test, expect, beforeEach, afterEach } from "bun:test"
import { buildConfigEnv } from "../src/server"

describe("buildConfigEnv", () => {
  const originalEnv = process.env.ACCURECODE_CONFIG_CONTENT

  beforeEach(() => {
    delete process.env.ACCURECODE_CONFIG_CONTENT
  })
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.ACCURECODE_CONFIG_CONTENT = originalEnv
    }
  })
  ...
  test("preserves existing ACCURECODE_CONFIG_CONTENT when spawning with new config", () => {
    // Simulate Accurecode having injected modes via ACCURECODE_CONFIG_CONTENT
    process.env.ACCURECODE_CONFIG_CONTENT = JSON.stringify({
      agent: {
        translate: { mode: "primary", prompt: "You are a translator" },
      },
      instructions: [".accurecode/rules/main.md"],
    })
    ...
    expect(parsed.instructions).toContain(".accurecode/rules/main.md")
  ...
  test("handles invalid JSON in existing ACCURECODE_CONFIG_CONTENT gracefully", () => {
    process.env.ACCURECODE_CONFIG_CONTENT = "invalid json {"
