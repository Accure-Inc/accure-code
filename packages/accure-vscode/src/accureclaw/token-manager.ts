/**
 * AccureChat access-token cache.
 *
 * Mirrors the web client pattern (see `apps/web/src/app/(app)/claw/accure-chat/token.ts`
 * in the cloud monorepo). The token is minted by the Accure gateway
 * (`accure.claw.chatCredentials`) and kept in memory with a 5-minute freshness
 * buffer. The gateway operation is a historical name — it used to return
 * Stream Chat credentials and was repurposed when Accure migrated to its own
 * accure-chat service.
 *
...
export class TokenManager {
  ...
  constructor(private readonly getClient: () => AccureClient | null) {}
  ...
  async getOrFetch(): Promise<ChatToken> {
    ...
    if (this.lastFailedAt && Date.now() - this.lastFailedAt < RETRY_BACKOFF_MS) {
      throw new Error("Accure chat token fetch on cooldown after recent failure")
    }
    if (!this.inflight) {
  ...
  private async fetch(): Promise<ChatToken> {
    ...
    if (!client) throw new Error("Accure backend not connected")
    const res = await client.accure.claw.chatCredentials()
    ...
    if (!res || errResponse || !res.data) {
      ...
      throw new Error(`accure-chat credentials fetch failed${detail ? `: ${detail}` : ""}`)
    }
    const data = res.data as Partial<ChatToken>
    const missing: string[] = []
    if (!data.token) missing.push("token")
    if (!data.expiresAt) missing.push("expiresAt")
    if (!data.accureChatUrl) missing.push("accureChatUrl")
    if (!data.eventServiceUrl) missing.push("eventServiceUrl")
    if (missing.length > 0) {
      throw new Error(
        `Malformed accure-chat credentials response: missing ${missing.join(", ")} (received keys: ${Object.keys(data).join(", ") || "<empty>"})`,
      )
    }
    return {
      token: data.token!,
      expiresAt: data.expiresAt!,
      accureChatUrl: data.accureChatUrl!,
