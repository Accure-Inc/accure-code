 ...
 * ## Why this exists
 *
 * The sidebar AccureProvider always uses `getWorkspaceDirectory()`, which returns
 ...
 *
 * ## How AccureProvider uses this
 *
 * Each AccureProvider instance can receive an explicit `projectDirectory` via
 * `AccureProviderOptions`. When set:
 *
...
/**
 * Resolve the effective project directory for a AccureProvider instance.
 *
 * @param override - Explicit directory from AccureProviderOptions. `undefined`
 ...
 *   `getWorkspaceDirectory(sessionId)` from AccureProvider).
 */
