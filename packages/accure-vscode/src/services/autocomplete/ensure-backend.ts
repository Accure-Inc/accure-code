 ...
 */
export function ensureBackendForAutocomplete(connection: AccureConnectionService): void {
  const enabled =
    vscode.workspace.getConfiguration("accure-code.autocomplete").get<boolean>("enableAutoTrigger") ?? true
  const dir = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath
  if (!enabled || !dir) return
  connection.connect(dir).catch((err) => {
    console.error("[Accure New] Autocomplete: Failed to start CLI backend:", err)
  })
}
