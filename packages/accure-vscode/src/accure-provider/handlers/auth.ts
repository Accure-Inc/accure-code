/**
 * Authentication handlers — extracted from AccureProvider.
 *
...
export async function handleLogin(ctx: AuthContext, attempt: number, getAttempt: () => number): Promise<void> {
  ...
  console.log("[Accure New] AccureProvider: 🔐 Starting login flow...")
  ...
  try {
    ...
    const { data: auth } = await ctx.client.provider.oauth.authorize(
      { providerID: "accure", method: 0, directory: dir },
      { throwOnError: true },
    )
    console.log("[Accure New] AccureProvider: 🔐 Got auth URL:", auth.url)
    ...
    console.log("[Accure New] AccureProvider: 🔐 Login successful")

    await ctx.disposeGlobal()

    // Step 3: Fetch profile and push to webview
    const { data: profile } = await ctx.client.accure.profile(undefined, { throwOnError: true })
...
export async function handleLogout(ctx: AuthContext): Promise<void> {
  ...
  try {
    console.log("[Accure New] AccureProvider: 🚪 Logging out...")
    await ctx.client.auth.remove({ providerID: "accure" }, { throwOnError: true })
    console.log("[Accure New] AccureProvider: 🚪 Logged out successfully")
    ...
    console.error("[Accure New] AccureProvider: ❌ Logout failed:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleSetOrganization(ctx: AuthContext, organizationId: string | null): Promise<void> {
  ...
  console.log("[Accure New] AccureProvider: Switching organization:", organizationId ?? "personal")
  try {
    await ctx.client.accure.organization.set({ organizationId }, { throwOnError: true })
  } catch (error) {
    console.error("[Accure New] AccureProvider: Failed to switch organization:", error)
    // Re-fetch current profile to reset webview state — best-effort
    try {
      const result = await ctx.client.accure.profile()
      ctx.postMessage({ type: "profileData", data: result.data ?? null })
    } catch (profileError) {
      console.error("[Accure New] AccureProvider: Failed to refresh profile after org switch error:", profileError)
    }
    return
  }
  ...
  try {
    ...
    console.error("[Accure New] AccureProvider: Failed to refresh agents after org switch:", error)
  }
}
...
export async function handleRefreshProfile(ctx: AuthContext): Promise<void> {
  ...
  console.log("[Accure New] AccureProvider: 🔄 Refreshing profile...")
  const result = await ctx.client.accure.profile().catch(() => ({ data: null }))
  ctx.postMessage({ type: "profileData", data: result.data ?? null })
}
