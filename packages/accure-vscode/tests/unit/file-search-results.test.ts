...
describe("mergeFileSearchResults", () => {
  ...
  it("preserves camel-case scoring for acronym-style queries", () => {
    const result = mergeFileSearchResults({
      query: "amprov",
      backend: [
        "packages/accure-docs/pages/contributing/architecture/onboarding-improvements.md",
        "packages/accure-vscode/src/agent-manager/AgentManagerProvider.ts",
      ],
      open: new Set(),
    })
    expect(result[0]).toBe("packages/accure-vscode/src/agent-manager/AgentManagerProvider.ts")
  })
})
