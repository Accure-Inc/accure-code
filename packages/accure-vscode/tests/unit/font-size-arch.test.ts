...
const TARGETS = [
  ...
  path.join(REPO, "packages/accure-ui/src/components"),
]
...
describe("webview font-size architecture", () => {
  ...
  it("uses scalable line heights in polished tool previews", () => {
    const files = [
      path.join(REPO, "packages/accure-ui/src/components/basic-tool.css"),
      path.join(REPO, "packages/accure-ui/src/components/message-part.css"),
    ]
  ...
  it("injects and live-broadcasts the webview font-size setting to all webview providers", () => {
    ...
    expect(
      missing.map(rel),
      `Webview providers that stay open must broadcast fontSizeChanged when accure-code.fontSize changes.\n` +
