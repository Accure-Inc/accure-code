 ...
 * they must be loaded from within packages/accure-ui/ where bun picks up the
...
const ASSISTANT_MESSAGE_FILE = path.join(
  MONOREPO_ROOT,
  "packages/accure-vscode/webview-ui/src/components/chat/AssistantMessage.tsx",
)
const CHAT_LAYOUT_FILE = path.join(MONOREPO_ROOT, "packages/accure-vscode/webview-ui/src/styles/chat-layout.css")
...
/**
 * Tool names that accure-vscode overrides or uses directly.
 * Sources:
...
describe("ToolRegistry tool name contract (runtime)", () => {
  it("all tools used by accure-vscode are registered in ToolRegistry", () => {
