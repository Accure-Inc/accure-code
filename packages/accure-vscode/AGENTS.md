...
Accure Code is an open source AI coding agent platform. It ships as a CLI and editor clients that all build on the same backend. This package (`packages/accure-vscode/`) is the **VS Code extension**.
...
- **`data-component` and `data-slot` attributes carry CSS styling** — accure-ui uses `[data-component]` and `[data-slot]` attribute selectors, not class names. Reuse existing component slots where available so shared styles apply consistently.
- **Prefer accure-ui styles**: Always reuse existing accure-ui CSS variables, tokens, and component styles instead of writing custom CSS. If a style doesn't exist in accure-ui yet, add it there and reuse it rather than inlining or duplicating styles in the webview.
- **Icons**: accure-ui has 75+ custom SVG icons in [`packages/ui/src/components/icon.tsx`](../../packages/ui/src/components/icon.tsx). To list all available icon names: `node -e "const c=require('fs').readFileSync('../../packages/ui/src/components/icon.tsx','utf8');[...c.matchAll(/^\\s{2}[\"']?([\\w-]+)[\"']?:\\s*\x60/gm)].map(m=>m[1]).sort().forEach(n=>console.log(n))"`. Icon names use both hyphenated (`arrow-left`) and bare-word (`brain`, `console`, `providers`) keys.
...
Generated screenshot baselines live under `packages/accure-docs/public/img/screenshot-tests/` and are referenced from docs as `/docs/img/screenshot-tests/...`. If a generated VS Code visual-regression screenshot is used in docs, add the docs usage to the `DOCS` map in `tests/visual-regression.spec.ts` and keep `tests/visual-regression.spec.mts` in sync while that file exists.
