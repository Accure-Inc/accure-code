...
| Rule source discovery | Auto-discovers from `.accurecode/rules/`, `.accurecoderules`, `.roorules`, `.clinerules`, mode-specific dirs, `AGENTS.md` | Only shows explicitly configured paths in `config.instructions` |
...
- **Auto-discovery**: Consider adding a section showing auto-discovered rule sources (`.accurecode/rules/`, `AGENTS.md`) that the CLI loads regardless of explicit `config.instructions` entries
