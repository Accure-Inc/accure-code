# Feature Parity Plan — Accure Code VS Code Extension (Rebuild)
...
| [Dedicated Output Channel](infrastructure/dedicated-output-channel.md) | General "Accure Code" output channel and centralized logging utility | P2 |
...
- [ ] **Bundle size audit** — With accure-ui and its transitive dependencies (shiki, marked, katex, dompurify, etc.) now bundled, measure `dist/webview.js` size and verify the total `.vsix` package size is acceptable
...
- Permissions flow through CLI's ask/reply model, not extension-side approval queues. Permissions are rendered through accure-ui's DataProvider pattern, not a standalone PermissionDialog.
